...
You might be thinking "38 milliseconds? That's nothing!" And you'd be right—if we were just talking about raw numbers.

But that's not what users experience. What they experience is that font flicker. That subtle delay. Research shows people perceive delays over 100ms, and one redirect puts you right there. More importantly, redirects cause visible symptoms as the browser makes multiple requests and reloads resources.

These tests were from a fast connection in New York. On 4G or 3G? You're looking at 200-500ms per redirect. And if you're far from GitHub's CDN edge, that cost multiplies. Every resource that redirects compounds the problem—your HTML redirects, some assets redirect, and suddenly you're talking hundreds of milliseconds.

The numbers tell one story. The font flicker tells the real one.

...
This is my [post](/posts/measuring-engineering-productivity/) on `curl` without a trailing slash (truncatated for effect).
...
So how do we actually measure this and see where we spend the time? The real secret is the `-w` option on `curl` which allows you to define an [output format](https://daniel.haxx.se/blog/2023/08/01/curl-write-out-to-files/) with your desired data. Perfect for blog posts!
